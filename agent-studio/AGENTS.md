...
- This repository is a near-direct port of the BezotCorp frontend, adapted to talk straight to `software-agent-sdk` / `agent_server` without the usual BezotCorp app backend.
- Frontend API adaptation lives mainly in `src/api/`:
  - `option-service` fabricates an OSS web-client config and reads models/providers through `@bezotcorp/typescript-client` LLM endpoints.
  - `settings-service` uses `@bezotcorp/typescript-client` settings APIs for persistence; reads schemas from `/api/settings/agent-schema` and `/api/settings/conversation-schema`, fetches settings with optional `X-Expose-Secrets: encrypted` header for conversation start payloads, and saves settings via PATCH with diffs.
  - `agent-server-conversation-service`, `event-service`, `agent-server-git-service`, and `skills-service` route local agent-server access through `@bezotcorp/typescript-client` rather than direct HTTP calls.
...
- Public skills are loaded from the `@bezotcorp/extensions` npm package at build time via `SKILLS_CATALOG` (exported from `@bezotcorp/extensions/skills`). The frontend's `SkillsService` maps catalog entries to `SkillInfo` objects and merges them with user/project skills fetched from the agent-server (with `load_public: false`). The agent-server no longer clones the extensions repo or uses `EXTENSIONS_REF` for public skills.
...
- The UI keeps most BezotCorp routes/layout intact, but hosted-only behavior (org, account management, integrations) has been removed via the fabricated OSS config because there is no separate app backend.
...
- **Keys**: hardcoded staging/prod keys in `telemetry.ts`; routed through `https://z.bezotcorp.dev`
- **Consent**: `localStorage["bezotcorp-telemetry-consent"]` via `useTelemetry` / `TelemetryConsentBanner`
...
- When the agent-studio dev launchers (`npm run dev` / `dev:minimal` / the published `agent-studio` binary) start a stack, they set a `VITE_RUNTIME_SERVICES_INFO` env var on the frontend describing which services are running and how the agent should reach them. The frontend forwards this verbatim as `AgentContext.system_message_suffix` on every `POST /api/conversations`, so conversations land with a `<RUNTIME_SERVICES>` block appended to the system prompt.
...
```json
{
  ...
  "services": {
    "agent_server": {
      "description": "The BezotCorp Agent Server this agent is running inside. ...",
    ...
    "frontend": {
      "kind": "vite",
      "description": "Vite dev server hosting the agent-studio frontend.",
      "url_from_agent": "http://localhost:3001"
    },
    "automation": {
      "description": "BezotCorp Automations service. All routes are mounted under '/api/automation'. Authenticate with header 'X-Session-API-Key: $BEZOTCORP_AUTOMATION_API_KEY'.",
...
All keys under `services` are optional and omitted when the corresponding service isn't running. `frontend.kind` is `"vite"` for dev launchers running the Vite dev server and `"static"` for stacks serving a pre-built `build/` directory (`dev:static`, the published `agent-studio` binary).

### Example `<RUNTIME_SERVICES>` block (dev with automation)

```
<RUNTIME_SERVICES>
You are running inside an agent-studio dev stack started in 'dev:automation' mode.
...
* Agent Server (you): http://localhost:18000
    The BezotCorp Agent Server this agent is running inside. Tool calls (terminal, file_editor, browser, etc.) execute here.
...
* Automation backend: http://localhost:18001
    BezotCorp Automations service. All routes are mounted under '/api/automation'. Authenticate with header 'X-Session-API-Key: $BEZOTCORP_AUTOMATION_API_KEY'.
    Docs:    http://localhost:18001/api/automation/docs
    OpenAPI: http://localhost:18001/api/automation/openapi.json
    Auth:    header 'X-Session-API-Key: $BEZOTCORP_AUTOMATION_API_KEY'
...
- `tests/e2e/live/utils/agent-server-conversation.ts` configures the running Agent Server before each live conversation by PATCHing `${LIVE_E2E_BACKEND_URL ?? "http://127.0.0.1:18100"}/api/settings` with LLM settings and low-risk conversation settings. LLM credentials are read from `LIVE_E2E_LLM_API_KEY`, `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, or `LLM_API_KEY`; CI defaults use `LIVE_E2E_LLM_BASE_URL` (default `https://llm-proxy.app.all-hands.dev`) and `LIVE_E2E_LLM_MODEL` (default `bezotcorp/claude-haiku-4-5-20251001`).
...
- **Production-fidelity launch**: The Playwright config (`playwright.mock-llm.config.ts`) starts the full `agent-studio` stack via `bin/agent-studio.mjs` — the same binary that `npx @bezotcorp/agent-studio` executes when users install the npm package. This means mock-LLM tests exercise the actual production path: pre-built static frontend + static-server.mjs + agent-server via uvx + automation backend via uvx + ingress proxy, all behind a single port.
...
- **State isolation**: `OH_CANVAS_SAFE_STATE_DIR=.tmp/mock-llm-state` isolates test state from the user's real `~/.bezotcorp/agent-studio/` directory. Both `STATE_DIR` (`.tmp/mock-llm-state`) and the automation DB dir (`.tmp/automation/`) are cleaned before each test run — the automation DB now lives outside STATE_DIR at `dirname(STATE_DIR)/automation/automations.db`, mirroring Docker's `~/.bezotcorp/automation/automations.db`.
...
- **Mock LLM server** (`tests/e2e/mock-llm/scripts/mock-llm-server.py`): Python HTTP server using bezotcorp-sdk's `TestLLM` to return scripted tool-call + text trajectories. Supports admin API endpoints for dynamic trajectory management:
...
- **Real automation backend**: The automation test uses the production automation backend (started by `bin/agent-studio.mjs`), NOT a mock server. Terminal `curl` commands from the agent hit the automation API through the ingress proxy at the test's `BACKEND_URL` (default `http://localhost:18300`). Auth uses the `X-Session-API-Key` header matching the stack's session key. The `mock-automation-server.py` file still exists as a reference but is not used by current tests.
- **Test helpers** (`tests/e2e/mock-llm/utils/mock-llm-helpers.ts`): Exports `registerTrajectory()`, `activateTrajectory()`, `resetMockLLM()`, `ensureMockLLMProfile()`, `getMockLLMRequests()` (fetches captured completion bodies from `GET /admin/requests`), `IMAGE_REPLY_TOKEN` + `MINIMAL_PNG_BASE64` (constants for the image-upload spec), ACP helpers (`configureAcpAgent()`, `verifyAcpAgentSettings()`, `resetToBezotCorpAgent()`, `ACP_REPLY_TOKEN`, `MOCK_ACP_SERVER_PATH`), and more.
...
- **Mock ACP server** (`tests/e2e/mock-llm/scripts/mock-acp-server.py`): A minimal stdio-based ACP agent that speaks JSON-RPC using the `acp` Python library (installed as a dependency of `bezotcorp-sdk`). Handles `initialize`, `session/new`, and `session/prompt`; sends a scripted `session/update` notification with `ACP_REPLY_TOKEN` in a text content block, then returns `stop_reason: "end_turn"`. The agent-server spawns it as a subprocess via `acp_command`. Accepts `--reply-token TOKEN` to customize the reply token.
...
- **Architecture**: The Docker config replaces the npm path's `bin/agent-studio.mjs` webServer with a `docker run --network host` command. The mock LLM server still runs on the host. On Linux (including CI), `--network host` lets the container share the host's network stack so all `127.0.0.1` URLs work identically. On macOS/Windows Docker Desktop (bridge networking), set `MOCK_LLM_AGENT_URL=http://host.docker.internal:<port>` so the agent-server inside Docker can reach the host-side mock LLM server.
...
- **Docker image**: Set `MOCK_LLM_DOCKER_IMAGE` to the image tag (default: `ghcr.io/bezotcorp/agent-studio:latest`). The container is started with `--rm --network host` and a unique `--name` for cleanup.
...
```bash
gh run download <run_id> --repo BezotCorp/bezot-agent-studio --name mock-llm-e2e-results --dir /tmp/artifacts
```
...
- **Published binary auth fix**: When users install the npm package globally (`npm install -g @bezotcorp/agent-studio`) and run `agent-studio`, the pre-built static frontend has NO `VITE_SESSION_API_KEY` baked in (npm publish runs `npm run build` with no such env var). The runtime session key is generated when the CLI launches and reaches the frontend via `scripts/static-server.mjs --session-api-key <key>`, which injects a `<head>` script that does two things: (a) sets `window.__AGENT_CANVAS_SESSION_API_KEY__ = <key>` — read by `getBakedSessionApiKey()` in `src/api/agent-server-config.ts` as a fallback when the env var is empty, symmetric with `__AGENT_CANVAS_AUTH_REQUIRED__` / `isAuthRequired()`; (b) writes the same key into `localStorage['bezotcorp-agent-server-config'].sessionApiKey`, always overwriting when the value differs, so any code path that still reads the legacy storage key (e.g. e2e fixtures) sees the live key. The window-global path is the load-bearing one — without it, `makeDefaultLocalBackend()` returns null on a fresh install, the backend registry seeds empty, and `root.tsx` traps the user behind the Manage Backends modal instead of onboarding. `scripts/dev-with-automation.mjs` and `scripts/dev-static.mjs` both pass `--session-api-key ${config.sessionApiKey}` when starting the static server.
...
- `@bezotcorp/typescript-client` should be pinned to a released version (currently a regular npm registry package, e.g. `1.24.3`) rather than an unreleased commit SHA; when agent-studio needs new client API, release the client first and then update the dependency to that version. Released versions should include the typed clients, agent-server version compatibility helpers, `WorkspacesClient`, `ConversationClient.switchLLM`, and subpath exports for `client/http-client`, `events/remote-events-list`, and `workspace/remote-workspace` needed by the agent-studio agent-server integration. `RemoteWorkspace.gitChanges`/`gitDiff` accept an optional `{ ref }` option; agent-studio passes `'HEAD'` so the changes panel reflects working-tree + index versus the latest commit (i.e. staged + unstaged) instead of a diff against the upstream/default branch.
- Any GitHub-hosted git dependency (today: `@bezotcorp/extensions` via the `github:BezotCorp/extensions#<sha>` shorthand) will be normalized by npm to `git+ssh://git@github.com/...` in `package-lock.json`. Vercel's build environment has no GitHub SSH key, so an ssh-pinned lockfile makes Vercel fall back to a stale cached tarball and the bundler can then fail with `[MISSING_EXPORT] ...` errors at bundle time. `scripts/vercel-install.sh` (wired up via `vercel.json`'s `installCommand`) defensively rewrites any `git+ssh://git@github.com/` resolved URLs in the lockfile to `git+https://github.com/` and adds matching `git config --global url..insteadOf` aliases before invoking `npm ci`, so the ssh-shaped lockfile that npm naturally produces still builds on Vercel. The rewrite is generic across all git deps, not specific to any one package. See GitHub issue #384 for the original failure (which was on `@bezotcorp/typescript-client` back when it was a git dep).
...
### Rule 1 -- Agent-server calls must use `@bezotcorp/typescript-client`

All calls that target the local agent-server (`/api/*`, `/server_info`, `/sockets`)
**must** go through typed client classes from `@bezotcorp/typescript-client`, **never**
raw `axios`, `fetch`, or the legacy shared `BEZOTCORP` axios instance.

Available clients and their subpath imports:

- `ConversationClient` -- `@bezotcorp/typescript-client/clients`
- `FileClient` -- `@bezotcorp/typescript-client/clients`
- `VSCodeClient` -- `@bezotcorp/typescript-client/clients`
- `ServerClient` -- `@bezotcorp/typescript-client/clients`
- `HttpClient` -- `@bezotcorp/typescript-client/client/http-client`
- `RemoteWorkspace` -- `@bezotcorp/typescript-client/workspace/remote-workspace`
- `RemoteEventsList` -- `@bezotcorp/typescript-client/events/remote-events-list`
...
```ts
// CORRECT
const ONBOARDING_COMPLETED_KEY = "bezotcorp-onboarded";
...
localStorage.setItem("bezotcorp-onboarded", "true");
queryClient.invalidateQueries({ queryKey: ["settings"] });
```

Already-named constants in this repo include `DEFAULT_WORKING_DIR` (`src/api/agent-server-config.ts`), `BEZOTCORP_I18N_NAMESPACE` (`src/i18n/index.ts`), `BUNDLED_BACKEND_ID` (backend registry), and the `*_QUERY_KEYS` helpers in `src/hooks/query/query-keys.ts`. Reuse these instead of re-inlining the literal.
...
- Generated files (`src/i18n/declaration.ts`, `public/locales/<lang>/bezotcorp.json`) are produced by `npm run make-i18n`; do not hand-edit, do not lint-target.
...
- `npm run dev:mock` needs MSW handlers for the direct agent-server routes used by the adapted frontend, not the original BezotCorp mock paths. Key routes that must stay covered are:
...
- Live `agent_server` compatibility quirks discovered during browser verification:
  - Latest `bezotcorp-agent-server` live-mode notes (verified against 1.18.1):
  ...
  - Runtime git panels should prefer the conversation's reported `workspace.working_dir` when present; falling back to `/workspace/project` can produce 500s like `Not a git repository` for direct local workspaces such as `/workspace/project/agent-studio`.
  - For development, `npm run dev` now uses `uvx` to run a temporary agent-server installation, so no permanent `uv tool install` is required. For standalone installations, `uv tool install -U --with bezotcorp-tools --with bezotcorp-workspace bezotcorp-agent-server` would expose the executable as `agent-server` (not `bezotcorp-agent-server`), possibly requiring `~/.local/bin` on `PATH`.
  ...
  - For local verification in this repo, setting `VITE_WORKING_DIR=/workspace/project/agent-studio` avoids initial Changes-tab 500s from pointing conversations at the non-repo parent `/workspace/project`.
...
- Git provider tokens are stored exclusively on the agent-server via `SecretsService` (`PUT /api/settings/secrets`). They are NOT mirrored to localStorage; the frontend reads which providers are connected from `settings.provider_tokens_set` (populated by `GET /api/settings`). Older notes about an `bezotcorp-agent-server-git-provider-tokens` localStorage key are obsolete — no such key is read or written anywhere in the codebase.
...
- Agent server connection settings now live at `Settings > Agent Server` (`/settings/agent-server`). The page reads deployment defaults from `VITE_BACKEND_BASE_URL` / `VITE_SESSION_API_KEY`, saves user overrides in the `bezotcorp-agent-server-config` localStorage key, and must stay reachable even when the backend compatibility probe fails so users can recover from missing or wrong backend configuration. Note: the backend registry seeds directly from the launcher-provided defaults.
- Auth modes for `agent-studio` (dev and production):
  - **Local mode** (default, no `--public` flag): A session API key is auto-generated and persisted to `~/.bezotcorp/agent-studio/session-api-key.txt`. The key is baked into the Vite dev server via `VITE_SESSION_API_KEY` or injected into static builds via `static-server.mjs --session-api-key`. Users never need to paste a key.
  - **Public mode** (`--public` flag): Requires `LOCAL_BACKEND_API_KEY` env var. The key is used as the agent-server session key (`OH_SESSION_API_KEYS_0`) but is NOT baked into the frontend (no `VITE_SESSION_API_KEY`, no `--session-api-key` to static-server). The frontend detects a 401 from `/server_info` via `isAgentServerAuthError()` and shows `ApiKeyEntryScreen` (`src/components/features/backends/api-key-entry-screen.tsx`). The screen reuses `BackendForm` with the host pre-filled (read-only) and prompts for the API key. On submit, the key is persisted to `localStorage['bezotcorp-agent-server-config']` and the page reloads.
  - Dev usage: `LOCAL_BACKEND_API_KEY=my-secret npm run dev -- --public`
  - Production usage: `LOCAL_BACKEND_API_KEY=my-secret npx @bezotcorp/agent-studio --public`
  - The `--public` flag is supported by both `scripts/dev-with-automation.mjs` (parsed in `parseArgs()`, propagated via `config.isPublic`) and `bin/agent-studio.mjs` (passed as `isPublic` to `main()`).
...
- README expectation: keep the first section as a concrete, chronological from-scratch quickstart for running this frontend against a real `bezotcorp-agent-server` (clone, install prerequisites, optional `.env`, run `npm run dev`).
...
- `scripts/dev-safe.mjs` uses `uvx` for temporary agent-server installation — no permanent `uv tool install` needed. Environment variables (highest precedence first):
  - `OH_AGENT_SERVER_LOCAL_PATH` — absolute path to a local `software-agent-sdk` checkout. Runs the local checkout via `uvx` with `--with-editable` for `bezotcorp-sdk`/`bezotcorp-tools`/`bezotcorp-workspace` and `--reinstall` for `bezotcorp-agent-server`, so SDK edits are picked up on restart. Highest precedence.
  ...
  - `OH_SECRET_KEY` — secret key for settings encryption; auto-generated and persisted to `~/.bezotcorp/agent-studio/secret-key.txt` on first run (same file Docker uses), ensuring dev mode and Docker share the same key when both mount the same `~/.bezotcorp` directory. Override with the env var to pin a specific key.
...
- Security: `scripts/dev-safe.mjs` and `scripts/dev-with-automation.mjs` auto-generate random API keys when needed and persist the defaults so static builds, localStorage, and restarted services stay in sync:
  - `SESSION_API_KEY` — 64-character hex (256-bit) for agent-server API authentication; persisted at `~/.bezotcorp/agent-studio/session-api-key.txt` unless overridden via env var
  - `AUTOMATION_LOCAL_API_KEY` — 64-character hex for automation backend auth; persisted at `~/.bezotcorp/agent-studio/automation-api-key.txt` unless overridden
  - `OH_SECRET_KEY` — 64-character hex (256-bit) for settings encryption; persisted at `~/.bezotcorp/agent-studio/secret-key.txt` unless overridden via env var. Same file used by `docker/entrypoint.sh`, so dev mode and Docker share the same key automatically.
...
- `scripts/dev-with-automation.mjs` runs the full stack: agent-server, automation backend (both via uvx), frontend server, and ingress proxy. It defaults to Vite when run directly, supports `--static` for an existing build, and supports `--dynamic` so wrappers that default static can opt back into Vite. Uses a standalone ingress proxy (`scripts/ingress.mjs`) to route traffic:
  ...
  - `scripts/check-sdk-version-sync.mjs` checks the released `bezotcorp-automation` package against `versions.agentServer` in `config/defaults.json`; these must always match — if the automation package's SDK dependencies differ from `agentServer`, the check fails.
  - Access points: `http://localhost:8000/` (main UI), `http://localhost:8000/api/automation/docs` (API docs)
  - Security: `AUTOMATION_LOCAL_API_KEY` defaults to a generated key persisted across restarts because static frontend builds bake it into `VITE_AUTOMATION_API_KEY`. Set the env var explicitly to rotate or pin it. The cipher key (`OH_SECRET_KEY`) is persisted at `~/.bezotcorp/agent-studio/secret-key.txt` (same file used by `docker/entrypoint.sh`); both modes share the same key automatically when using the same `~/.bezotcorp` directory.
...
- Bundle/dev-graph hygiene (Tier 1 cleanup landed):
  - `src/i18n/translation.json` (~1 MB) is imported only by `src/i18n/resources.ts`, which `src/i18n/index.ts` re-exports as `translationResources` for the `@bezotcorp/agent-studio/i18n` subpath. The re-export is a `export … from` plus `/* @__PURE__ */` annotation, so rollup drops the JSON from the app build (the prod `custom-toast-handlers` chunk went from ~909 KB to ~74 KB). Do not move the JSON import back into `src/i18n/index.ts` — that immediately re-bundles all translations into every chunk that imports `i18n`.
...
- BezotCorp repo bootstrap files live under `.bezotcorp/`:
  - `.bezotcorp/setup.sh` installs `uv` (via `curl -LsSf https://astral.sh/uv/install.sh | sh`) if not present, installs frontend dependencies with `npm ci` when needed, creates `.env` from `.env.sample` if missing, appends `VITE_WORKING_DIR` for this repo when unset, and generates `src/i18n/declaration.ts` via `npm run make-i18n`.
  - `.bezotcorp/hooks.json` registers `.bezotcorp/hooks/on_stop.sh` as a Stop hook so BezotCorp runs the local quality gate (`npm run lint` and `npm test`) before finishing.
- GitHub PR-review automation should stay aligned with the current BezotCorp repo conventions: keep the review workflow at `.github/workflows/pr-review-by-bezotcorp.yml`, keep the companion `.github/workflows/pr-review-evaluation.yml`, auto-run on newly opened non-draft PRs and `ready_for_review` events from established contributors, still support the `review-this` label / `bezotcorp-agent` / `all-hands-bot` reviewer triggers, use the BezotCorp app LLM proxy defaults, and use the dual-trigger pattern (`pull_request` for same-repo PRs, `pull_request_target` for forks) so workflow changes can self-verify without widening fork secret exposure.
- The repo now includes `.agents/skills/custom-codereview-guide.md`, adapted from `BezotCorp/software-agent-sdk`, to force PR reviews to always leave either an APPROVE or COMMENT review instead of silently finishing with no review object.
...
- Library i18n is now namespace-scoped under `bezotcorp`: `src/i18n/index.ts` exports `BEZOTCORP_I18N_NAMESPACE`, `translationResources`, and `waitForI18n()`, `scripts/make-i18n-translations.cjs` emits `public/locales/<lang>/bezotcorp.json`, standalone `src/entry.client.tsx` explicitly awaits i18n init, and host apps can register bundles via the `@bezotcorp/agent-studio/i18n` subpath export.
...
- Bundle/dev-graph hygiene (Tier 1 cleanup landed):
  - `src/i18n/translation.json` (~1 MB) is imported only by `src/i18n/resources.ts`, which `src/i18n/index.ts` re-exports as `translationResources` for the `@bezotcorp/agent-studio/i18n` subpath. The re-export is a `export … from` plus `/* @__PURE__ */` annotation, so rollup drops the JSON from the app build (prod `custom-toast-handlers` chunk: 909 KB -> 74 KB; `conversation` chunk: 728 KB -> 392 KB). Do not move the JSON import back into `src/i18n/index.ts` — that immediately re-bundles all translations into every chunk that imports `i18n`.
  - The environment-switch overlay is split: lightweight store/triggers live in `components/features/backends/environment-switch-store.ts`; the React component lives in `environment-switch-overlay.tsx` (re-exports the store API for back-compat). Eagerly-mounted callers (e.g. `backend-selector.tsx`) MUST import trigger helpers from the store, not the overlay file. The overlay is `React.lazy`'d from `routes/root-layout.tsx`.
...
- **Centralized config**: `config/defaults.json` is the single source of truth for version pins (agent-server, automation, automation SDK), port defaults, persistence paths, and package names. All consumers read from this file:
  ...
  - Docker: a `config-gen` build stage converts the JSON to `/opt/agent-studio/defaults.env` (shell-sourceable); `entrypoint.sh` sources it at startup.
