 ...
 */
function groupEventsByPhase(events: BezotCorpEvent[]): BezotCorpEvent[][] {
  const phases: BezotCorpEvent[][] = [];
  let currentPhase: BezotCorpEvent[] = [];
 ...
 */
export function usePlanPreviewEvents(allEvents: BezotCorpEvent[]): Set<string> {
  return useMemo(() => {
