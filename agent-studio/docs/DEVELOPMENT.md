...
`OH_BASH_EVENTS_DIR`, and `OH_VSCODE_PORT` values under `.bezotcorp-dev/`, and
keeps its tmux sockets under `~/.bezotcorp/agent-studio/tmux` (via
`TMUX_TMPDIR`), so it does not collide with other local or cloud-backed
BezotCorp sessions. If `$HOME` is on a filesystem that does not support Unix
...
`OH_AGENT_SERVER_LOCAL_PATH` must be an absolute path to a `software-agent-sdk` checkout containing the `bezotcorp-agent-server`, `bezotcorp-sdk`, `bezotcorp-tools`, and `bezotcorp-workspace` workspace packages. The agent-server itself is rebuilt from local source on each start (`uvx --reinstall`); the other workspace packages are installed editable, so their source changes take effect without a rebuild.
